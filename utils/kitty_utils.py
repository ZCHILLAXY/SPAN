...
def cropAndCenterPC_train(PC, sample_box, gt_box, offset=0, scale=1.0, normalize=False):
    ...
    center_box_gt = [new_box_gt.center[0],new_box_gt.center[1],new_box_gt.center[2],new_box_gt.wlh[2],new_box_gt.wlh[0],new_box_gt.wlh[1],new_box_gt.orientation.axis[2] * new_box_gt.orientation.radians]
    center_box_gt = np.array(center_box_gt)
    # label_reg = np.tile(label_reg,[np.size(new_label),1])

    if normalize:
        new_PC.normalize(sample_box.wlh)
    return new_PC, center_box_gt
...
def cropAndCenterPC_test(PC, sample_box, gt_box, offset=0, scale=1.0, normalize=False):
    ...
    if normalize:
        new_PC.normalize(sample_box.wlh)
    return new_PC, new_label, label_reg, new_box, new_box_gt, trans, rot_mat
